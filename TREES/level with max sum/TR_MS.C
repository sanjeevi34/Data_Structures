...
int n_of_leaves(struct node_tree* root)
{
	int leaves = 0;
	struct node_tree* temp;
	enqueue(root);
...
int max_level(struct node_tree* root)
{
	int maxlevel=0,maxsum=0,level=0,currentsum=0;
	struct node_tree* temp;
	enqueue(root);
	enqueue(NULL);
	while(!isEmpty())
...
int main()
{
	...
	printf("\nThe no. of leaves in the tree is : %d",max_level(root));
	getch();
	return 0;
}