...
int max_path_sum(struct node_tree *root)
{
	int left_sum, right_sum,max_path_value;
	if(root == NULL)
		return 0;
	left_sum = max_path_sum(root->left);
	right_sum = max_path_sum(root->right);
	max_path_value = max(left_sum,right_sum)+root->data;
...
int main()
{
	...
	printf("%d <-- Maximum path sum",max_path_sum(root));
	getch();
	return 0;
}